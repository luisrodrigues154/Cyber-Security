...
static void incnfocusclient(const Arg *arg);
...
void
incnfocusclient(const Arg *arg)
{
	Client *c = selmon->sel;
	int targetX = c->x;
	int targetW = c->w + arg->i;
	if((c->x + c->w + selmon->gappov) == selmon->ww) targetX -= arg->i;
	else 											 targetX -= arg->i/2;

	FILE *fp = fopen("/home/waza/Documents/log", "w+");

	fprintf(fp, "c->x %d\nc->w %d\n\ntargetX %d\ntargetW %d\n", c->x, c->w, targetX, targetW);
	fclose(fp);
	resizeclient(c, targetX, c->y, targetW, c->h); 
	//arrange(selmon->clients);
}
